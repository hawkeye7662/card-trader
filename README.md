...
A Discord.js bot for posting Clash card trade offers. Members run `/trade elixir`, `/trade dark-elixir`, `/trade builder-base`, or `/trade super-troop`, choose up to three cards they have and want from private multi-select menus, then publish a rendered trade card. Only the creator can close an offer.
...
Edit `src/cards.ts` to replace the placeholder names and IDs. Put a PNG or WebP file for each card in `assets/cards/` and set that card's `imagePath` (for example, `assets/cards/elixir-archer.png`). Missing artwork deliberately renders as a polished placeholder tile, so the bot works before assets are ready.
